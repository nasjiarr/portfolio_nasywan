<script>
	...
	onMount(() => {
		// Strictly disable custom cursor on touch devices and for prefers-reduced-motion
...
{#if isEnabled}
	...
	<div
		bind:this={dotEl}
		class="fixed top-0 left-0 pointer-events-none z-[9999] w-2 h-2 -ml-1 -mt-1 rounded-full bg-accent dark:bg-accent-dark transition-opacity duration-200 {isVisible
			? 'opacity-100'
	...
	<div
		bind:this={ringEl}
		class="fixed top-0 left-0 pointer-events-none z-[9998] flex items-center justify-center -translate-x-1/2 -translate-y-1/2 rounded-full border border-accent/50 dark:border-accent-dark/50 transition-all duration-300 ease-out {isVisible
			? 'opacity-100'
